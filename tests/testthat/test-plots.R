test_that("CPplot result stores plot, slopes, and data", {
  ...
  fit <- cp_plot(df, e_hat = "e_hat", tau_hat = "tau")
...
test_that("cp_plot supports treated and control point labels", {
  ...
  fit <- cp_plot(df, e_hat = "e_hat", tau_hat = "tau", treatment = "z")
...
test_that("cp_plot can estimate default inputs from outcome, treatment, and covariates", {
  set.seed(10)
  n <- 120
...
test_that("cp_plot does not accept the old ehat argument", {
  ...
  expect_error(cp_plot(df, ehat = "e_hat", tau_hat = "tau"), "unused argument")
})
...
test_that("local_cp_plot returns weighted local CP diagnostics", {
  ...
  fit <- local_cp_plot(df, e_hat = "e_hat", tau_c_hat = "tau_c", pi_c_hat = "pi_c", group = "z")
  expect_s3_class(fit, "CPplot_result")
  expect_equal(levels(fit$data_used$group), c("Control", "Treated"))
